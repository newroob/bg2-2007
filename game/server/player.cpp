...
int CBasePlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
		...
		( !attacker->IsSolidFlagSet(FSOLID_TRIGGER)) )
	{
		...
		//ApplyAbsVelocityImpulse( force ); //BG2 - Stop pushing people around. -HairyPotter
	}
...
bool CBasePlayer::StartObserverMode(int mode)
{
	...
	//Don't set the player to EF_NODRAW - the client can determine //BG2 - Hmm.. -HairyPotter
...
void CBasePlayer::Precache( void )
{
	...
	//PrecacheScriptSound( "Player.Death" ); //BG2 - Doesn't exist in scripts. -HairyPotter
	PrecacheScriptSound( "Player.PlasmaDamage" );
	//PrecacheScriptSound( "Player.SonicDamage" ); //BG2 - Doesn't exist in scripts. -HairyPotter
