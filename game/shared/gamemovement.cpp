...
void CGameMovement::Accelerate( Vector& wishdir, float wishspeed, float accel )
{
	...
	//currentspeed = mv->m_vecVelocity.Dot(wishdir);
	currentspeed = mv->m_vecVelocity.Length(); //BG2 - Keep the player speeds the same when against walls. -HairyPotter
...
// XBOX SERVER ONLY
	...
	//BG2 - This keeps the menu from shaking around whn you create a game. I believe this is because the
