...
ConVar mp_respawnwavetime( "mp_respawnwavetime", "10.0", FCVAR_REPLICATED | FCVAR_CHEAT | FCVAR_DEVELOPMENTONLY, "Time between respawn waves." );
...
void cc_TestStalemate( const CCommand& args )
{
	if ( UTIL_IsCommandIssuedByServerAdmin() )
	{
		CTeamplayRoundBasedRules *pRules = dynamic_cast<CTeamplayRoundBasedRules*>( GameRules() );

		int iReason = STALEMATE_TIMER;		
		if ( args.ArgC() == 3 )
		{
			iReason = atoi( args[2] );
		}
		...
		if ( pRules )
		{
			pRules->SetStalemate( iReason, atoi(args[1]) != 0 );
		}
	}
}
static ConCommand mp_teststalemate( "mp_teststalemate", cc_TestStalemate, "Test the stalemate mode. Parameter: <0/1>. If 1, the map will reset at the end.", FCVAR_CHEAT );
...
void CTeamplayRoundBasedRules::State_Think_TEAM_WIN( void )
{
	if( gpGlobals->curtime > m_flStateTransitionTime )
	{
		...
		if ( !CheckTimeLimit() && !CheckWinLimit() && !CheckMaxRounds() )
		{
			PreviousRoundEnd();
			State_Transition( GR_STATE_PREROUND );
		}
...
//			round timer expired 
//-----------------------------------------------------------------------------
void CTeamplayRoundBasedRules::SetStalemate( int iReason, bool bForceMapReset /* = true *//* )
{
	if ( InStalemate() )
		return;
